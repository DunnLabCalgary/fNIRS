...
if ~exist([Data.Path.study_path filesep 'Processed'])
        mkdir([Data.Path.study_path filesep 'Processed']);
end
...
for S_count = 1:numel(Data.SOI) % SOI = subjects of interest (subjects selected in gui)
    ...
    %% validate Data shape:
    X.s = X.s(:,1);%Only take first column of S
   % if ~isfield(X.SD, 'SpatialUnit')
   %     X.SD.SpatialUnit = 'cm';
    %    warndlg('No spatial unit specified in SD variable of data. Assuming cm.')
   ...
   % end
    ...
    if ~exist([Data.Path.study_path filesep 'Processed' filesep sub])
        ...
        mkdir([Data.Path.study_path filesep 'Processed' filesep sub filesep 'Preprocessing']);
    end
    ...
    if ~exist('w','var')
        ...
        waitbar((.50*S_count)/length(Data.SOI),w,['Assessing signal quality - ' strrep(sub, '_', ' ')]);
    end
    ...
    if PreUI.Figs.sigQuality == 1
        ...
        figure(h1);
        saveas(gca,[Data.Path.study_path filesep 'Processed' filesep sub filesep 'Preprocessing' filesep 'SignalQuality'],'png');
        close
    end
    ...
    if ~exist('w','var')
        w = waitbar(0 ,['Performing Motion Correction - ' strrep(sub, '_', ' ')]);
    else
        waitbar((.50*S_count)/length(Data.SOI),w,['Performing Motion Correction - ' strrep(sub, '_', ' ')]);
    end
    ...
    if ~exist('w','var')
        ...
        waitbar((.75*S_count)/length(Data.SOI),w,['Processing Time-Series - ' strrep(sub, '_', ' ')]);
    end
    

    
    for lm = 1:length(X.SD.Lambda)
        DPF(lm) = CalcDPF (Data.Demographics.Age(S_count),X.SD.Lambda(lm));
    end
    ...
    if isempty(GC) 
        warndlg(['All channels where discarded for subject:' sub])
    ...
    if ~exist([Data.Path.study_path filesep 'Processed' filesep sub filesep 'CorrectedSignal'])
        mkdir([Data.Path.study_path filesep 'Processed' filesep sub filesep 'Preprocessing' filesep 'CorrectedSignal']);
    end
    for ii = 1:length(Ch)
        if ismember(Ch(ii),GC)
            ...
            if PreUI.Figs.corrSig == 1
                ...
                saveas(gca,[Data.Path.study_path filesep 'Processed' filesep sub filesep 'Preprocessing' filesep 'CorrectedSignal' filesep ...
                    'Ch' num2str(Ch(ii))],'png');close
            end
        end
    end
    ...
    if ~exist('w','var')
        ...
        waitbar(status,w, ['Saving data - ' strrep(sub, '_', ' ')]);
    end
...
%     % Preprocessed data goes in subject processed folder
    ...
    save([Data.Path.study_path filesep 'Processed' filesep sub filesep 'Preprocessing' filesep 'Processed.mat'],'Proc');
...
function [QualityCheck] = performQualityCheck(study_path, PreUI, X, sub)
    ...
    if PreUI.Figs.DataQuality == 1    
        saveas(gcf,[study_path filesep 'Processed' filesep sub filesep 'Preprocessing' filesep 'QualityCheck'],'png');close;
    end
    
    if PreUI.Figs.cardiacFreq == 1
        saveas(gcf,[study_path filesep 'Processed' filesep sub filesep 'Preprocessing' filesep 'CardiacPlot'],'png');close;
    end 
