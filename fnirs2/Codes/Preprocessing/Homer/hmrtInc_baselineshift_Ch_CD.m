...
lpf = 2;
% if lpf>fs/2
%     warning(['Sample frequency of ' num2str(fs) ' Hz is too low for low pass filtering at 2 Hz. No low pass filter was performed.'])
% end
for ww = 1:size(dod,2)
    clearvars -except ww tInc d ml fs t dod tMotion SD dodorg p iqr SNR_Thre SNR_Thre2 FrameSize_sec 
    s1 =  dod(:,ww) ;
    
    % Chris added if clause to not lpf if sampling fz is low (ie. NIRX)
    lpf = 2;
    if lpf>fs/2
        lpf = fs/2;
    else
        [s1,ylpf] = hmrBandpassFilt( s1, fs, 0, lpf );
    end
    
    [s2,ylpf] = hmrBandpassFilt( s1, fs, 0, 0.5);
...
%             warning(['Sample frequency of ' num2str(fs) ' Hz is too low for low pass filtering at 2 Hz. No low pass filter was performed.'])
            ...
            [s1,ylpf] = hmrBandpassFilt( stemp, fs, 0, lpf );
        end
