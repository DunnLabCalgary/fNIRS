...
for S_count = 1:length(Data.SOI) % loop counter for all subjects selected
    ...
    load([procFile filesep 'Preprocessing' filesep 'Processed.mat']); % load subject's preprocessed data struct
    cd(procFile);
    ...
    for rc = 1:length(regionComparison)
        if regionComparison(rc)
            ...
            if PostUI.Fig
                ...
                WFile = ['WCohMatrix_' F1 '_' F2 '_Hz_' char(regionNames(rc)) '.png'];
                saveas(gcf,WFile)
                close
            end
