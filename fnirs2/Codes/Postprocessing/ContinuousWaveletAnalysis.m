function [X] = ContinuousWaveletAnalysis(FZband,Proc, RegionalData, Hemo, stimRects, procFile, sub)
% profile on
...
%% Initialize
    ...
    X = regionalComparisonWaveletAnalysis(ChannelsX, ChannelsY, Proc,si, f1, f2, fs, Hemo, 1, procFile, sub, wlabel);

% profile viewer
% profile off
end

function [X] = regionalComparisonWaveletAnalysis(ChannelsX, ChannelsY, Proc,si, f1, f2, fs, Hemo, detailLog, procFile, sub, wlabel)
...
for hb = 1:numel(Hemo)
    ...
    if Hb == "StO2"
        ...
        %save the new proc var with sto2 in it.
        Proc.dc(:,4,:) = d;
        save(procFile, 'Proc')
